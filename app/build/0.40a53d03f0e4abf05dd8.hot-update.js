...
var areas = function areas() {
	...
	switch (action.type) {
		...
		case "area_item_edit":
			console.log(state[action.area_key].items[action.item._id]);
			state[action.area_key].items[action.item._id] = action.item;
			console.log(state[action.area_key].items[action.item._id]);
			return __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_assign___default()({}, state);
		case "area_items_set":
...
//# sourceMappingURL=0.40a53d03f0e4abf05dd8.hot-update.js.map