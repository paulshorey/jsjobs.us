...
var Results = function (_Component) {
   ...
   */
			...
			for (var i in items_object) {
				...
				// except
				if (item._status === _this.props.my_items_status) {
...
var mapStateToProps = function mapStateToProps(data, ownProps) {
	...
	var newProps = {
		area: {
			...
			my_items_applied: data.my.applied.find(function (item) {
				return item.status === "applied";
			}),
			my_items_intrigued: data.my.intrigued.find(function (item) {
				return item.status === "intrigued";
			}),
			my_items_ignored: data.my.ignored.find(function (item) {
				return item.status === "ignored";
			}),
			my_items_status: data.my.status || "new"
		}
	};
	return newProps;
};
...
//# sourceMappingURL=0.dc56590d403eca438dca.hot-update.js.map