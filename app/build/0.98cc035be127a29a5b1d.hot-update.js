...
var Results = function (_Component) {
	...
	__WEBPACK_IMPORTED_MODULE_3_babel_runtime_helpers_createClass___default()(Results, [{
		key: "item_mutate_and_save",
		value: function item_mutate_and_save(item, changes) {
			// MUTABLE change assign() directly to item --- this assignment is temporary for the current scope --- it is separate from Redux action, which performs its own immutable Object.assign({}, item)
			var mutated_item = __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_assign___default()(item, { _status: "applied" });
			// OK, below is normal
			this.props.dispatch(__WEBPACK_IMPORTED_MODULE_11__data_actions__["c" /* my_item_save */](mutated_item, this.props.area_key));
		}
		/*
  	view
  */
...
var mapStateToProps = function mapStateToProps(data, ownProps) {
	...
	// for (const it in data.my.items) {
	// 	const item = data.my.items[it];
	// 	if (ownProps.items[item._id]) {
	// 		delete ownProps.items[item._id];
	// 	}
	// }
...
//# sourceMappingURL=0.98cc035be127a29a5b1d.hot-update.js.map